...
//!     the M2 mouse-capture acceptance harness instead.
//!   * `KMWARP_M5_DEMO=1` (macOS only) — bypass the normal server and run
//!     the M5 keyboard-capture acceptance harness instead.
...
async fn main() -> Result<()> {
    ...
    // M2 / M5 acceptance demos: short-circuit the normal server when the
    // hook is set. macOS-only because they depend on `CGEventTap`. On other
    // platforms the env vars are ignored.
    #[cfg(target_os = "macos")]
